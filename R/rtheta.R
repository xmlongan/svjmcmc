...
#' The posterior
#' \deqn{\begin{matrix}
#'       P(\theta|v_{0:N},y_{1:N},J_{1:N},I_{1:N})
...
#' rtheta(0.01,1,v,y, jsize, jtime, mu,k,sigma_v,rho,h)
rtheta <- function(prior_mu,prior_var,v,y,jsize,jtime,mu,k,sigma_v,rho,h) {
  N = length(y)
  for (n in 1:N) {
    # y_n <- v_{n-1}
