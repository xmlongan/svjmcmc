...
#' \deqn{\begin{matrix}
#'       P(\rho|v_{0:N},y_{1:N},J_{1:N},I_{1:N})
...
#' \end{matrix}}
#' where \eqn{x_n\triangleq y_n-\mu h+\frac{1}{2}v_{n-1}h - J_nI_n},
#' \eqn{P(\rho)} denotes
...
#' rrho(rho_old,v,y,jsize,jtime,mu,k,theta,sigma_v,h)
rrho <- function(rho_old,v,y,jsize,jtime,mu,k,theta,sigma_v,h) {
  # compute the sample correlation between the Brownian increments
  rho0 = get_rho0(v,y,jsize,jtime,mu,k,theta,sigma_v,h)
  # propose a new rho
  ...
  for (n in 1:N) {
    # y_n <- v_{n-1},v_n
    x_n = y[n] - mu*h + v[n]*h/2 - jsize[n]*jtime[n]
    # v_n <- v_{n-1}
