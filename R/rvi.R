...
#' The posterior
#' \deqn{\begin{matrix}
#'        P(v_i|v_{0:i-1,i+1:N},y_{1:N},J_{1:N},I_{1:N})
#'        &\propto& P(v_{0:N},y_{1:N}|J_{1:N},I_{1:N})\\
#'        &\propto& P(v_{i+1}|v_i,y_{i+1},J_{i+1},I_{i+1})
#'        \cdot P(y_{i+1}|v_i,J_{i+1},I_{i+1})
#'        \cdot P(v_i|v_{i-1},y_i,J_i,I_i)
#'       \end{matrix}}
#' Introduce notation
...
#' thus, we have
#' \deqn{v_{i+1}|v_i,y_{i+1},J_{i+1},I_{i+1}
#'       \sim \mathcal{N}(\tilde{v}_{i+1}, \sigma_v^2(1-\rho^2)v_ih),\quad
#'       P(v_{i+1}|v_i,y_{i+1},J_{i+1},I_{i+1})
#'       = \frac{1}{\sqrt{2\pi(1-\rho^2)v_ih}\sigma_v}
#'         e^{-\frac{(v_{i+1}-\tilde{v}_{i+1})^2}{2\sigma_v^2(1-\rho^2)v_ih}},}
#' \deqn{y_{i+1}|v_i,J_{i+1},I_{i+1}
#'       \sim \mathcal{N}(\mu h - v_ih/2 + J_{i+1}I_{i+1}, v_ih),\quad
#'       P(y_{i+1}|v_i,J_{i+1},I_{i+1})
#'       = \frac{1}{\sqrt{2\pi v_ih}}
#'         e^{-\frac{(y_{i+1}-\mu h + v_ih/2 - J_{i+1}I_{i+1})^2}{2v_ih}},}
#' \deqn{v_{i}|v_{i-1},y_{i},J_i,I_i
#'       \sim \mathcal{N}(\tilde{v}_{i}, \sigma_v^2(1-\rho^2)v_{i-1}h),\quad
#'       P(v_{i}|v_{i-1},y_{i},J_i,I_i)
...
#' @param jsize_i jump size \eqn{J_i}.
#' @param jsize_ip1 jump size \eqn{J_{i+1}}.
#' @param jtime_i jump indicator \eqn{I_i}, 0 or 1.
#' @param jtime_ip1 jump indicator \eqn{I_{i+1}}, 0 or 1.
...
#' rvi(v[i+1],v[i],v[i+2],y[i],y[i+1],jsize[i],jsize[i+1],jtime[i],jtime[i+1],mu,k,theta,sigma_v,rho,h)
rvi <- function(vi_old,vim1,vip1,yi,yip1,jsize_i,jsize_ip1,jtime_i,jtime_ip1,mu,k,theta,sigma_v,rho,h) {
  # propose new vi
  ...
  #
  # v_i <- v_{i-1},y_i,jsize_i,jtime_i
  mu1 = vim1 + k*(theta-vim1)*h + sigma_v*rho*(yi-mu*h+vim1*h/2-jsize_i*jtime_i)
  var1 = sigma_v^2*(1-rho^2)*vim1*h
  ratio1 = exp(- (vi_new - mu1)^2 / (2*var1)
               + (vi_old - mu1)^2 / (2*var1))
  #
  # y_{i+1} <- v_i,jsize_{i+1},jtime_{i+1}
  ratio2 = exp(- (yip1 - mu*h + vi_new*h/2 - jsize_ip1*jtime_ip1)^2 / (2*vi_new*h)
               + (yip1 - mu*h + vi_old*h/2 - jsize_ip1*jtime_ip1)^2 / (2*vi_old*h))
  #
  # v_{i+1} <- v_i,y_{i+1},jsize_{i+1},jtime_{i+1}
  mu3_new = vi_new + k*(theta-vi_new)*h + sigma_v*rho*(yip1-mu*h+vi_new*h/2-jsize_ip1*jtime_ip1)
  mu3_old = vi_old + k*(theta-vi_old)*h + sigma_v*rho*(yip1-mu*h+vi_old*h/2-jsize_ip1*jtime_ip1)
