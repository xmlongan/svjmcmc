...
#' The posterior
#' \deqn{\begin{matrix}
#'        P(v_N|v_{0:N-1},y_{1:N},J_{1:N},I_{1:N})
...
#' @param jsize_N jump size \eqn{J_N}.
#' @param jtime_N jump indicator \eqn{I_N}, 0 or 1.
...
#' N = length(y)
#' jsize_N = rnorm(1, mean=0, sd=0.5)
#' jtime_N = rbinom(1,size=1, prob=0.01)
#' rvN(v[N+1],v,y,jsize_N,jtime_N,mu,k,theta,sigma_v,rho,h)
rvN <- function(vN_old,vNm1,yN,jsize_N,jtime_N,mu,k,theta,sigma_v,rho,h) {
  # propose new vN
  ...
  #
  # v_N <- v_{N-1},y_N,jsize_N,jtime_N
  muN = vNm1 + k*(theta-vNm1) * h + sigma_v * rho * (yN - mu*h + vNm1*h/2 - jsize_N*jtime_N)
