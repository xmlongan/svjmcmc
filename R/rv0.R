...
#' @param jsize_1 jump size \eqn{J_1}.
#' @param jtime_1 jump or not \eqn{I_1}, 0 or 1.
...
rv0 <- function(v0_old,prior_shape,prior_rate,v1,y1,jsize_1,jtime_1,mu,k,theta,sigma_v,rho,h) {
  # propose new v0
  ...
  #
  # y_1 <- v_0,jsize_1,jtime_1
  ratio1 = exp(-(y1 - mu*h + v0_new*h/2 - jsize_1*jtime_1)^2 / (2*v0_new*h)
               +(y1 - mu*h + v0_old*h/2 - jsize_1*jtime_1)^2 / (2*v0_old*h))
  #
  # v_1 <- v_0,y_1,jsize_1,jtime_1
  m_new = v0_new + k*(theta-v0_new) * h + sigma_v * rho * (y1-mu*h + v0_new*h/2 - jsize_1*jtime_1)
  m_old = v0_old + k*(theta-v0_old) * h + sigma_v * rho * (y1-mu*h + v0_old*h/2 - jsize_1*jtime_1)
