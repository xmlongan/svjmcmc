...
#' where
#' \deqn{x_n\triangleq \frac{y_n+v_{n-1}h/2-\rho\sqrt{v_{n-1}h}\epsilon_n^v
#'      -J_nI_n}{h}
#'      ,\quad \sqrt{v_{n-1}h}\epsilon_n^v
#'      = \frac{v_n-v_{n-1}-k(\theta-v_{n-1})h}{\sigma_v}.}
#' The posterior
#' \deqn{\begin{matrix}
#'       P(\mu|v_{0:N},y_{1:N},J_{1:N},I_{1:N})
...
#' rmu(0, 0.1^2, v, y, jsize, jtime, k, theta, sigma_v, rho, h)
rmu <- function(prior_mu,prior_var,v,y,jsize,jtime,k,theta,sigma_v,rho,h) {
  N = length(y)
  for (n in 1:N) {
    # v_n <- v_{n-1}
    std_eps_v = ( v[n+1] - v[n] - k*(theta - v[n])*h )/sigma_v # v[1]: v_0
    # y_n <- v_{n-1},v_n
    x_n = ( y[n] + v[n]*h/2 - rho*std_eps_v - jsize[n]*jtime[n] )/h
