...
#' volatility samples.
#' \deqn{\epsilon_n^y = \frac{y_n-\mu h + \frac{1}{2}v_{n-1}h -J_nI_n}{\sqrt{v_{n-1}h}}
#' ,\qquad
...
#' get_rho0(v,y,jsize,jtime,mu,k,theta,sigma_v,h)
get_rho0 <- function(v,y,jsize,jtime,mu,k,theta,sigma_v,h) {
  eps_y = rep(0,length(y))
  eps_v = rep(0,length(y))
  for (n in 1:length(y)) {
    eps_y[n] = (y[n] - mu*h + v[n]*h/2 - jsize[n]*jtime[n])/(sqrt(v[n]*h))
