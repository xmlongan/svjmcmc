...
double cget_rho0(NumericVector v, NumericVector y,
  ...
  double rho0;
  for(int n=0; n < N; n++) {
    eps_y[n] = (y[n] - mu * h + v[n] * h/2 - jsize[n]*jtime[n]) / std::sqrt(v[n] * h);
...
//' @rdname rrho
// [[Rcpp::export]]
double crrho(double rho_old, NumericVector v, NumericVector y,
              NumericVector jsize, NumericVector jtime,
              double mu,
   ...
   rho0 = cget_rho0(v, y, jsize, jtime, mu, k, theta, sigma_v, h);
   // propose a new rho
