...
// [[Rcpp::export]]
NumericVector crSVJ(double v_0, double n_segment, NumericVector par, int N,
                        double h=1){
   double mu,k,theta,sigma,rho, lmbd, mu_j, sigma_j;
   mu = par[0]; k = par[1]; theta = par[2]; sigma = par[3]; rho = par[4];
   lmbd = par[5]; mu_j = par[6]; sigma_j = par[7];
   ...
   for (int i = 0; i < N; i++) {
     ...
     if (n > 0) {
       for (int j=0; j < n; j++) {
         J_n += R::rnorm(mu_j, sigma_j);
       }
     }
     y = mu*h - IV/2 + rho*I + std::sqrt(1-rho*rho)*I2 + J_n;
     y_series[i] = y;
   }
   return y_series;
 }
...
// [[Rcpp::export]]
void gen_data_SVJ(std::string par_name,NumericVector par,int N,int N_rep,
   ...
   for (int i = 0; i < N_rep; i++) {
     ...
     y_series = crSVJ(theta,n_segment,par,N,h);
