...
double crv0(double v0_old, double prior_shape, double prior_rate, double v1,
             double y1, double jsize_1, double jtime_1,
             double mu, double k, double theta, double sigma_v,
             double rho, double h) {
   ...
   //
   // y_1 <- v_0
   ratio1 = std::exp( -std::pow(y1 - mu*h + v0_new*h/2 - jsize_1*jtime_1, 2) / (2*v0_new*h) +
     std::pow(y1 - mu*h + v0_old*h/2 - jsize_1*jtime_1, 2) / (2*v0_old*h) );
   //
   // v_1 <- v_0,y_1
